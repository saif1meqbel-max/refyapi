...
SECTIONS = [
  ("intro", None, (
    "REF YAPI'nin (\"REF YAPI\") kullanıcıların (\"Kullanıcı(lar)\") arzusuna bağlı olarak web sitesi (\"Web Sitesi\") üzerinden elde ettiği kişisel verileri toplarken ve kullanırken izlediği prensipler bu Gizlilik Politikası'nda düzenlenmektedir.",
    "This Privacy Policy sets out the principles followed by REF YAPI (\"REF YAPI\") when collecting and using personal data obtained from users (\"User(s)\") through the website (\"Website\") at their discretion.",
    "Настоящая Политика конфиденциальности определяет принципы, которых придерживается REF YAPI (\"REF YAPI\") при сборе и использовании персональных данных пользователей (\"Пользователь(и)\") через веб-сайт (\"Веб-сайт\") по их желанию.",
  )),
  ("collect", "Toplanabilecek Kişisel Veriler", (
    """REF YAPI, Web Sitesi üzerinde, Kullanıcıların Web Sitesi'ne erişimi ve yapacakları işlemlere bağlı olarak Kullanıcı'nın;
...
<p>Kişisel Verilerin Korunması Kanunu'nun 3 ve 7. maddesi uyarınca, geri döndürülemeyecek şekilde anonim hale getirilen veriler, anılan kanun hükümleri uyarınca kişisel veri olarak kabul edilmeyecek ve bu verilere ilişkin işleme faaliyetleri işbu Gizlilik Politikası hükümleri ile bağlı olmaksızın gerçekleştirilecektir.</p>""",
    """On the Website, depending on Users' access and transactions, REF YAPI may collect the User's:
...
<p>Pursuant to Articles 3 and 7 of the Personal Data Protection Law, data irreversibly anonymised shall not be regarded as personal data, and processing of such data may be carried out without being bound by this Privacy Policy.</p>""",
    """На Веб-сайте, в зависимости от доступа и действий Пользователей, REF YAPI может собирать:
...
<p>В соответствии со ст. 3 и 7 Закона, необратимо анонимизированные данные не считаются персональными, и их обработка может осуществляться без ограничений настоящей Политики.</p>""",
  ...
  ("cookies", '"Cookie" Kullanımı', (
    """REF YAPI tarafından, Kullanıcılar'ın Web Sitesi'ndeki dolaşımları sırasında ziyaret edilen bölümler, tıklanan alanlar gibi veriler otomatik olarak toplanmaktadır. "Cookie" adlı teknoloji kullanılarak elde edilen bu veriler istatistiksel bilgilerdir. Bu teknoloji ile amaçlanan, Kullanıcılar'ın ziyaret ettikleri bölümlere ait içeriği, siteye ilk ziyaretlerinden itibaren Kullanıcı için daha kolay ulaşılır kılmaktır. Tarayıcıların pek çoğu başta teknik iletişim dosyası olan bu "cookie"leri kabul eder biçimde tasarlanmıştır, ancak Kullanıcılar dilerlerse teknik iletişim dosyasının gelmemesi veya teknik iletişim dosyasının gönderildiğinde ikaz verilmesini sağlayacak biçimde tarayıcı ayarlarını her zaman için değiştirebilirler.

<p>REF YAPI, online davranışsal reklamcılık ve pazarlama yapılabilmesi amacıyla Kullanıcılar'ın Web Sitesi'ndeki davranışlarını tarayıcıda bulunan bir "cookie" ile ilişkilendirme ve görüntülenen sayfa sayısı, ziyaret süresi ve hedef tamamlama sayısı gibi metrikleri temel alan yeniden pazarlama listeleri tanımlama hakkını haizdir. Sonrasında Kullanıcı'ya Web Sitesi'nde ya da görüntülü reklam ağındaki diğer sitelerde ilgi alanlarına göre hedefe yönelik reklam içeriği gösterilebilir.</p>""",
    """During Users' navigation on the Website, data such as sections visited and areas clicked are collected automatically by REF YAPI using "cookie" technology. This data is statistical. The purpose is to make content in visited sections more accessible from the User's first visit. Most browsers accept cookies by default; Users may change browser settings to block cookies or receive warnings when cookies are sent.

<p>REF YAPI may associate Users' Website behaviour with browser cookies and define remarketing lists based on metrics such as pages viewed, visit duration and goal completions for online behavioural advertising and marketing. Targeted advertising may then be shown on the Website or other sites in display networks according to interests.</p>""",
    """При навигации по Веб-сайту REF YAPI автоматически собирает данные о посещённых разделах и кликах с помощью технологии «cookie». Эти статистические данные делают контент более доступным с первого визита. Браузеры обычно принимают cookies по умолчанию; Пользователь может изменить настройки для блокировки или предупреждений.

<p>REF YAPI вправе связывать поведение на Веб-сайте с cookie и формировать списки ремаркетинга для поведенческой рекламы. Пользователю могут показываться таргетированные объявления на Веб-сайте или других площадках.</p>""",
  )),
  ("purpose", "Verilerin Kullanılma Amacı", (
    """REF YAPI, toplanan kişisel verileri Kullanıcı'nın Web Sitesi'nden faydalanabilmesi, Web Sitesi'ne üyelik söz konusu ise üyelik kaydının gerçekleştirilmesi, sunduğu hizmetlerin iyileştirilmesi, hizmetin geliştirilmesi, yeni hizmetlerin tanıtımı ve bilgilendirilmesi, bu kapsamda Kullanıcı'ya gerekli bilgilendirilmelerin yapılması, Kullanıcı ile temas kurulması ve sunduğu hizmetlerin doğasından kaynaklanan yükümlülüklerin yerine getirilmesi için işleyebilir.

<p>Söz konusu kişisel veriler REF YAPI raporlama ve iş geliştirme faaliyetleri kapsamında işlenebilecek, Kullanıcı'nın kimliği ifşa edilmeden çeşitli istatistiksel değerlendirmeler yapma, veri tabanı oluşturma ve pazar araştırmalarında bulunma amacıyla da kullanılabilecektir. Kullanıcı'nın ayrıca onay vermesi halinde söz konusu bilgiler REF YAPI ve işbirliğinde olduğu kişiler tarafından doğrudan pazarlama yapmak amacıyla işlenebilecek, saklanabilecek, üçüncü kişilere iletilebilecek ve söz konusu bilgiler üzerinden çeşitli uygulama, ürün ve hizmetlerin tanıtımı, bakım ve destek faaliyetlerine ilişkin bildirimlerde bulunma amacıyla Kullanıcı ile iletişime geçilebilecektir.</p>

<p>REF YAPI ayrıca, Kişisel Verilerin Korunması Kanunu'nun 5 ve 8. maddeleri uyarınca ve/veya ilgili mevzuattaki istisnaların varlığı halinde kişisel verileri Kullanıcı'nın ayrıca rızasını almaksızın işleyebilecek ve üçüncü kişilerle paylaşabilecektir. Bu durumların başlıcaları aşağıda belirtilmiştir:</p>
...
<li>Kullanıcı ile REF YAPI arasında herhangi bir sözleşmenin kurulması veya ifasıyla doğrudan doğruya ilgili olması kaydıyla, kişisel verilerin işlenmesinin gerekli olması</li>
...
<li>Kullanıcı'nın temel hak ve özgürlüklerine zarar vermemek kaydıyla, REF YAPI'nin meşru menfaatleri için veri işlenmesinin zorunlu olması</li>
</ul>""",
    """REF YAPI may process collected personal data so that the User can benefit from the Website, complete membership registration where applicable, improve and develop services, promote new services, provide necessary information, contact the User and fulfil obligations arising from the nature of its services.
...
<p>Under Articles 5 and 8 of the Personal Data Protection Law and applicable exceptions, REF YAPI may process and share personal data without separate consent, including where:</p>

<ul class="legal-list">
<li>Explicitly required by law</li>
<li>Necessary to protect life or physical integrity where consent cannot be given</li>
<li>Necessary for establishment or performance of a contract between User and REF YAPI</li>
...
<li>Necessary for legitimate interests of REF YAPI without harming fundamental rights and freedoms</li>
</ul>""",
    """REF YAPI обрабатывает персональные данные для использования Веб-сайта, регистрации (если применимо), улучшения услуг, информирования, связи с Пользователем и выполнения обязательств.
...
<li>необходимо для законных интересов REF YAPI</li>
</ul>""",
  )),
  ("sharing", "Verilerin Paylaşımı", (
    """REF YAPI, Kullanıcılar'a ait kişisel verileri ve bu kişisel verileri kullanarak elde ettiği yeni verileri Web Sitesi kapsamında Kullanıcı'ya sunulan hizmetlerin ifası amacıyla hizmetlerinden faydalandığı üçüncü kişilere, söz konusu hizmetlerin temini amacıyla sınırlı olmak üzere aktarabilecektir. Bu kapsamda REF YAPI, Kullanıcı deneyiminin geliştirilmesi (iyileştirme ve kişiselleştirme dahil), Kullanıcı'nın güvenliğini sağlamak, hileli ya da izinsiz kullanımları tespit etmek, operasyonel değerlendirme araştırılması, Web Sitesi veya REF YAPI hizmetlerine ilişkin hataların giderilmesi ve işbu Gizlilik Politikası'nda veya Kullanıcı'ya sunulan sair gizlilik metinlerinde yer alan amaçlardan herhangi birisini gerçekleştirebilmek için Kullanıcı verilerini dış kaynak hizmet sağlayıcıları, barındırma hizmet sağlayıcıları ("hosting" servisleri), hukuk büroları, araştırma şirketleri, çağrı merkezleri gibi üçüncü kişiler ile paylaşabilecektir.
...
<p>The User accepts that such third parties may store personal data on servers anywhere in the world for the purposes stated above.</p>""",
    """REF YAPI может передавать персональные данные третьим лицам, оказывающим услуги, в объёме, необходимом для работы Веб-сайта — провайдерам, хостингу, юридическим и исследовательским компаниям, колл-центрам для улучшения опыта, безопасности, выявления мошенничества и иных целей Политики.
...
<p>Пользователь соглашается, что такие третьи лица могут хранить данные на серверах в любой точке мира в указанных целях.</p>""",
  ...
  ("rights", "Kullanıcı'nın Verilere Erişim Hakkı ve Düzeltme Talepleri", (
    """Kullanıcı, REF YAPI'e başvurarak kendisiyle ilgili:
...
<p>REF YAPI, yukarıda yer alan talepler uyarınca, gerekçeli olumlu/olumsuz yanıtını, yazılı veya dijital ortamdan gerçekleştirebilir. Taleplere ilişkin gerekli işlemler için ücret alınmaması esastır. Bununla birlikte, işlemlerin bir maliyet gerektirmesi halinde, Kişisel Verilerin Korunması Kurulu tarafından, Kişisel Verilerin Korunması Kanunu'nun 13. maddesine göre belirlenen tarife üzerinden ücret talep edilmesi mümkündür.</p>

<p>Kullanıcı, REF YAPI'e kendisi tarafından sağlanmış olan, işbu Gizlilik Politikası'na konu bilgilerin tam, doğru ve güncel olduğunu, bu bilgilerde herhangi bir değişiklik olması halinde bunları derhal güncelleyeceğini taahhüt eder. Kullanıcı'nın güncel bilgileri sağlamamış olması halinde REF YAPI'nin herhangi bir sorumluluğu olmayacaktır.</p>

<p>Kullanıcı, herhangi bir kişisel verisinin REF YAPI tarafından kullanılamaması ile sonuçlanacak bir talepte bulunması halinde REF YAPI'nin kendisine taahhüt ettiği hizmetlerden tam olarak faydalanamayabileceğini, bu kapsamda doğacak her türlü sorumluluğun kendisine ait olacağını kabul ve beyan eder.</p>""",
    """The User may apply to REF YAPI to:
...
<p>REF YAPI may respond in writing or digitally with reasoned approval or rejection. Fees are generally not charged; where costs arise, fees may be requested per the tariff under Article 13 of the Personal Data Protection Law.</p>

<p>The User undertakes that information provided is complete, accurate and current and will update changes promptly. REF YAPI bears no liability if current information is not provided.</p>
...
<p>If a request results in data no longer being usable, the User accepts they may not fully benefit from promised services and bears resulting responsibility.</p>""",
    """Пользователь вправе обратиться в REF YAPI чтобы:
...
<p>REF YAPI отвечает письменно или в цифровом виде. Плата обычно не взимается; при необходимости — по тарифу ст. 13 Закона.</p>

<p>Пользователь обязуется предоставлять полные и актуальные данные и обновлять их. REF YAPI не несёт ответственности при неактуальных данных.</p>
...
<p>Если запрос делает использование данных невозможным, Пользователь принимает ограничение услуг и ответственность за последствия.</p>""",
  ...
  ("retention", "Kişisel Verilerin Saklama Süresi", (
    """REF YAPI, Kullanıcı tarafından sağlanan kişisel verileri, sunduğu hizmetlerin mahiyetinden kaynaklanan yükümlülüklerin yerine getirilmesi amacıyla, hizmetlerin sağlandığı süre boyunca saklayacaktır.

<p>Buna ek olarak, REF YAPI, Kullanıcı ile arasında doğabilecek herhangi bir uyuşmazlık durumunda, uyuşmazlık kapsamında gerekli savunmaların gerçekleştirilebilmesi amacıyla sınırlı olmak üzere ve ilgili mevzuat uyarınca belirlenen zamanaşımı süreleri boyunca kişisel verileri saklayabilecektir.</p>""",
    """REF YAPI will retain personal data provided by the User for the duration of services to fulfil obligations arising from the nature of those services.
...
<p>Additionally, in the event of disputes, data may be retained for limitation periods under applicable law to enable necessary defences.</p>""",
    """REF YAPI хранит персональные данные в течение срока оказания услуг для выполнения связанных обязательств.
...
<p>Дополнительно при спорах данные могут храниться в пределах сроков исковой давности по закону.</p>""",
  ...
  ("security", "Veri Güvenliğine İlişkin Önlemler, Taahhütler ve Sorumluluk", (
    """REF YAPI, ilgili mevzuatta belirlenen veya işbu Gizlilik Politikası'nda ifade edilen şartlarda:
...
<p>Site üzerinde başka uygulamalara link verilmesi halinde, REF YAPI uygulamaların gizlilik politikaları ve içeriklerine yönelik bünyesinde herhangi bir sorumluluk taşımamaktadır.</p>

<p>REF YAPI kişisel verilerin yukarıdaki şartlar çerçevesinde kullanılması sonucu meydana gelebilecek zararlardan dolayı sorumluluk kabul etmez.</p>""",
    """REF YAPI undertakes to take necessary technical and administrative measures and conduct audits to ensure appropriate security so that personal data is not processed unlawfully, accessed unlawfully, or lost, under applicable law and this Privacy Policy.

<p>Where the Site links to other applications, REF YAPI accepts no responsibility for their privacy policies or content.</p>

<p>REF YAPI accepts no liability for damages that may arise from use of personal data within the above framework.</p>""",
    """REF YAPI обязуется принимать технические и организационные меры и проводить проверки для обеспечения надлежащей защиты персональных данных от незаконной обработки, доступа и утраты.

<p>На ссылки на другие приложения REF YAPI не несёт ответственности за их политики и контент.</p>

<p>REF YAPI не несёт ответственности за ущерб от использования данных в указанных рамках.</p>""",
  )),
  ("changes", "Gizlilik Politikası'ndaki Değişiklikler", (
    """Web Sitesi'nde sunulan hizmetlerden yararlananlar bütün bu şartları okumuş ve kabul etmiş sayılırlar. REF YAPI, Gizlilik Politikası hükümlerini önceden haber vermeksizin değiştirme hakkını saklı tutar. Güncel Gizlilik Politikası, Kullanıcı'ya herhangi bir yöntemle sunulduğu tarihte yürürlük kazanır.""",
    """Those who use services on the Website are deemed to have read and accepted all these terms. REF YAPI reserves the right to change this Privacy Policy without prior notice. The updated Privacy Policy takes effect on the date it is made available to the User by any means.""",
    "Пользователи услуг Веб-сайта считаются прочитавшими и принявшими все условия. REF YAPI вправе изменять Политику без предварительного уведомления. Обновлённая Политика вступает в силу с даты её предоставления Пользователю любым способом.",
  )),
]
...
<head>
  ...
  <title>Gizlilik Politikası — REF YAPI</title>
...
{sections}    <p class="legal-contact">{L("İlgili diğer belgeler: ", "Related documents: ", "Связанные документы: ")}<a href="cookie-policy.html">{L("Çerez Politikası", "Cookie Policy", "Политика cookie")}</a> · {L("Gizlilik talepleri: ", "Privacy requests: ", "Запросы по конфиденциальности: ")}<a href="mailto:info@refyapi.com">info@refyapi.com</a></p>
  </div>
</section>
